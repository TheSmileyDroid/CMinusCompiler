...
std::string getExpKindStr(ExpressionKind kind) {
    switch (kind) {
        case OpK: return "Operador";
        case ConstK: return "Constante";
        case IdK: return "ID";
        case CallK: return "Chamada";
...
void printTree(TreeNode* tree) {
    ...
    while (tree != nullptr) {
        ...
        if (tree->nodeKind == StatementK) {
            std::cout << "Statement: " << getStmtKindStr(tree->kind.stmt) << "\n";
        }
        else if (tree->nodeKind == ExpressionK) {
            std::cout << "Expression: " << getExpKindStr(tree->kind.exp);
